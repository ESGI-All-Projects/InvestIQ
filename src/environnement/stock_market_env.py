...
class MultiStockRepartitionTradingEnv(gym.Env):
    def __init__(self, stock_data):
        super(MultiStockRepartitionTradingEnv, self).__init__()
        ...
        # Action space: Proportion of investment for each stock (should sum to 1)
        ...
        self.observation_space = spaces.Box(
            low=-np.inf,
            high=np.inf,
            shape=(self.n_stocks * (self.n_features + 2),),
            dtype=np.float32
        )
    ...
    def reset(self, **kwargs):
        ...
        self.positions = np.zeros(self.n_stocks)  # Number of shares held
        self.invested_amount = np.zeros(self.n_stocks)  # Amount invested in each stock
        self.previous_portfolio_value = 100000  # Initial portfolio value (cash only)
        self.portfolio_value = 100000
        self.baseline_quota = 100000 / np.sum(self.stock_data.iloc[self.current_step, :self.n_stocks])
    ...
    def _take_action(self, action):
        # Ensure action proportions sum to 1
        assert (np.sum(action) - 1 < 1e-4)
        # action = np.clip(action, 0, 1)
        # action = action / np.sum(action)

        # Calculate the amount to invest in each stock
        total_investment = self._calculate_portfolio_value()
        new_investment = total_investment * action

        for i in range(self.n_stocks):
            stock_price = self.stock_data.iloc[self.current_step, i]

            self.positions[i] = new_investment[i] / stock_price
            self.invested_amount[i] = new_investment[i]


    def _calculate_portfolio_value(self):
        self.portfolio_value = np.sum(self.positions * self.stock_data.iloc[self.current_step, :self.n_stocks])
        return self.portfolio_value
